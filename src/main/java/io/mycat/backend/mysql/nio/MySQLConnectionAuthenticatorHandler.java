 ...
 * @author mycat
 */
public class MySQLConnectionAuthenticatorHandler implements NIOHandler {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(MySQLConnectionAuthenticatorHandler.class);
	private final MySQLConnection source;
	private final ResponseHandler listener;

	public MySQLConnectionAuthenticatorHandler(MySQLConnection source,
	 ...
	 * 在MySQL5.5的版本之后可以客户端插件式加密(这个MyCat实现)
	...
	public void handle(byte[] data) {
		try {
			switch (data[4]) {
			case OkPacket.FIELD_COUNT:	// 如果是OkPacket，检查是否认证成功
				...
				break;
			case ErrorPacket.FIELD_COUNT:	// 如果为ErrorPacket，则认证失败
			...
			case EOFPacket.FIELD_COUNT:	// 如果是EOFPacket，则为MySQL 4.1版本，是MySQL323加密
				auth323(data[3]);
				break;
			default:
